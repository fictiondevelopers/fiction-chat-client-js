...
const FictionChatClient = ({
  ...
  const connectWebSocket = () => {
    ...
    wsRef.current.onclose = () => {
      ...
      if (reconnectAttemptsRef.current < 5) {
        ...
        setWsError(
          "Messages may not update in real-time due to connection issues"
        );
      }
    };
  };
  ...
  const fetchConversations = async () => {
    try {
      ...
      const response = await fetch(
        chatServerUrl + "?method=get-conversations",
        {
          headers: {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        ...
        const response = await fetch(
          chatServerUrl +
            "?method=get-messages&conversationId=" +
  ...
  const fetchAvailableUsers = async () => {
    try {
      const token = getToken();
      const response = await fetch(
        chatServerUrl + "?method=get-available-users-to-chat",
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  if (error) {
    return (
      <div
        className={`flex flex-col items-center justify-center min-h-screen w-full  ${contentContainerClassName} `}
      >
          ...
          {conversations.map((conversation) => (
            ...
            >
              <div className="relative">
                ...
                <div
                  className={`h-12 w-12 rounded-full ${getRandomColor()} flex items-center justify-center ${
                    conversation.other_user?.profilePicture ? "hidden" : ""
                  }`}
                >
            ...
            <div className="p-4 border-b flex items-center space-x-3">
              {selectedConversation.other_user?.profilePicture ? (
                <>
                  ...
                  />
                  <div
                    className={`h-10 w-10 rounded-full ${getRandomColor()} hidden items-center justify-center`}
                  >
                ...
                <div
                  className={`h-10 w-10 rounded-full ${getRandomColor()} flex items-center justify-center`}
                >
                  <span className="text-gray-600">
                    {getInitials(selectedConversation.other_user?.fullname)}
                  </span>
              ...
              <h2 className="font-medium">
                {selectedConversation.other_user?.fullname}
              </h2>
            </div>
                    ...
                    <div className="flex-shrink-0">
                      {message.sender.profilePicture ? (
                        <>
                          ...
                          />
                          <div
                            className={`h-8 w-8 rounded-full ${getRandomColor()} hidden items-center justify-center`}
                          >
                        ...
                        <div
                          className={`h-8 w-8 rounded-full ${getRandomColor()} flex items-center justify-center`}
                        >
                          <span className="text-gray-600 text-sm">
                            {getInitials(message.sender.fullname)}
                          </span>
                ...
                >
                  {user.profile_picture ? (
                    <>
                      ...
                      />
                      <div
                        className={`w-10 h-10 rounded-full ${getRandomColor()} hidden items-center justify-center`}
                      >
                        <span className="text-gray-600">
                    ...
                    <div
                      className={`w-10 h-10 rounded-full ${getRandomColor()} flex items-center justify-center`}
                    >
                      <span className="text-gray-600">
                        {getInitials(user.fullname)}
                      </span>
